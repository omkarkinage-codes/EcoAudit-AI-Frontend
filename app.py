...
# --- FORCE FILE SYSTEM ARCHITECTURE INITIALIZATION ---
def init_databases():
...
# ==========================================
    ...
    st.write("---")
    choice = st.segmented_control("Action", ["Sign In", "Sign Up"], default=st.session_state.auth_action)
    ...
    if st.session_state.auth_action == "Sign In":
        ...
        if st.button("Login", type="primary", use_container_width=True):
            ...
            if not user_row.empty:
                ...
                st.error("User email matching parameters not found.")
    else:
        if st.session_state.selected_role is None:
            ...
            st.markdown(f"### Register: {st.session_state.selected_role}")
            ...
            location = st.text_input("Location City").strip()
            meta_1 = st.text_input("License Code or Business Sector Specifier").strip()
...
# ==========================================
    ...
    # HIGH-SPEED BUTTON-BASED SIDEBAR
    with st.sidebar:
        ...
        if st.session_state.current_role == "Industrial Seller (Factory / Plant)":
            ...
            if st.button("🛠️ Manage Posts", use_container_width=True): st.session_state.current_tab = "🛠️ Manage Posts"
        else:
        ...
        if st.button("⚙️ My Account", use_container_width=True): st.session_state.current_tab = "⚙️ My Account"
        
        st.write("<br><br>" * 3, unsafe_allow_html=True)
    ...
    if st.session_state.current_tab == "📊 Dashboard":
        ...
        m1, m2 = st.columns(2)
        m1.metric("Active Market Entries", str(len(df_market)))
        m2.metric("Connected Corporate Nodes", str(len(df_users)))
        st.write("---")
        
        c1, c2 = st.columns([1.5, 1])
        with c1:
            st.markdown("### 📋 Live Open Market Listings")
            if df_market.empty:
                ...
                for _, row in df_market.iterrows():
                    st.markdown(f"""
                        <div class='eco-card'>
                            ...
                            <small style='color: #64748B;'>Company Base: {row['sender_company']} | {row['timestamp']}</small>
                        </div>
                    """, unsafe_allow_html=True)
        with c2:
            ...
            for _, row in df_users.iterrows():
                if row['email'] != st.session_state.current_user:
                    ...
                    if st.button(f"Connect to {row['company_name']}", key=f"dir_{shared_room_hash[:8]}"):
                        st.session_state.active_room_key = shared_room_hash
                        st.session_state.current_tab = "💬 Communication Terminal"
                        st.rerun()

    # --- TAB 2: SELLER DISPATCH ENGINE ---
    elif st.session_state.current_tab == "🚀 Dispatch Byproduct":
        st.title("🚀 Post New Byproduct Material")
        st.write("---")
        
        # CLEAR ON TRIGGER ENGINE VIA LOCAL SESSION VARIABLES
        if "w_log" not in st.session_state: st.session_state.w_log = ""
        if "m_map" not in st.session_state: st.session_state.m_map = ""
        if "q_val" not in st.session_state: st.session_state.q_val = ""

        raw_log = st.text_area("Paste Raw Warehouse Log / Factory Notes:", value=st.session_state.w_log, key="raw_log_box")
        material_type = st.text_input("Material Category Mapping", value=st.session_state.m_map, key="mat_box")
        quantity = st.text_input("Total Weight / Volume (Specify 'kg' or 'tons')", value=st.session_state.q_val, key="qty_box")
        ...
        if st.button("Post Listing to Market", type="primary", use_container_width=True):
            if raw_log.strip() and quantity.strip():
                ...
                # TRIGGER FORCIBLE TEXT BLANK STATE
                st.session_state.w_log = ""
                st.session_state.m_map = ""
                st.session_state.q_val = ""
                
                # PIPELINE AUTOMATION HOOK
                try:
                    requests.post("https://ecoaudit-ai.app.n8n.cloud/webhook-test/d70c8a5d-55ca-4673-a2a8-fe4b26f9c23f", 
                                  json={"message": raw_log, "material": material_type, "weight": quantity, "sender": st.session_state.current_user}, timeout=2)
    ...
    # --- TAB 3: SELLER DATA MODIFICATION WORKSPACE ---
        ...
        if my_posts.empty:
            st.info("You have no active entries up for collection.")
        else:
            for idx, row in my_posts.iterrows():
                with st.container():
                    st.markdown(f"**Item ID:** `{row['id']}` | **Category:** {row['material_type']}")
                    new_qty = st.text_input("Modify Quantity Tracking Metric:", value=row['quantity'], key=f"edit_{row['id']}")
        ...
        if df_market.empty:
            ...
            for _, row in df_market.iterrows():
                ...
                with col_y:
                    ...
                    if st.button("Claim & Chat", key=f"claim_{row['id']}", use_container_width=True):
                        st.session_state.active_room_key = shared_room_hash
                        st.session_state.current_tab = "💬 Communication Terminal"
                        st.rerun()

    # --- TAB 5: CENTRAL PEER-TO-PEER COMM TERMINAL ---
        ...
        if "active_room_key" not in st.session_state:
        ...
        if not st.session_state.active_room_key:
            ...
            sel_label = st.selectbox("Select Secure Business Channel:", list(rooms_map.values()), index=list(rooms_map.keys()).index(st.session_state.active_room_key) if st.session_state.active_room_key in rooms_map else 0)
    ...
    # --- TAB 6: INFRASTRUCTURE PROFILE VIEW ---
        ...
        st.markdown(f"""
            <div class='eco-card'>
                ...
                <p><strong>Account Email:</strong> {user_meta['email']}</p>
